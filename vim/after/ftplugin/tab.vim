...
export def g:TonarMais()
    :g/\(^\|\s\)[A-G][#b]\=[Mmº°]\=\([0-9][0-9]\=\(M\|-\|+\)\=\)\=\($\|\/\|(\|\(maj\|sus\|add\|dim\)\|\s\+\([A-G]\|)\|[0-9]x\|$\)\)/Mais()
    feedkeys('``')
enddef

export def g:TonarMenos()
    :g/\(^\|\s\)[A-G][#b]\=[Mmº°]\=\([0-9][0-9]\=\(M\|-\|+\)\=\)\=\($\|\/\|(\|\(maj\|sus\|add\|dim\)\|\s\+\([A-G]\|)\|[0-9]x\|$\)\)/Menos()
    feedkeys('``')
enddef
...
nnoremap <buffer> +       :call TonarMais() <CR><CR>
nnoremap <buffer> --      :call TonarMenos() <CR><CR>
